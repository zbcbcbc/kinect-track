...
void broadcast_pos_json(zmq::socket_t &s, int x, int y,double depth, double area) {
    if (x == 0 && y == 0) {
        string message = "[]";
        ...
        string message = pos_to_json(x, y, depth, area,0, 11);
        cout << message << endl;
...
int main(int, char **)
{
	...
	*/
    while(c != 'q')
    {
        ...
        // Blend together some of the edges and get rid of any noise, find contours.
		...
		searchWindow=boundingRect(contours[maxContour]);
		roi=img(searchWindow);
		get_image(roi);
		inRange(hsv,Scalar(0, sMin, vMin, 0),:Scalar(181, 256, vMax, 0), mask);
		calcHist(&hsv, 1, &channel, mask, hist, 1, &bins, ranges, true, false);
		minMaxLoc(hist, NULL, &histMax);
		hist *= histMax ? 255.0 / histMax : 0.0;
		calcBackProject(&img, 1, &channel, hist, bp, ranges, 1, true);
		RotatedRect foundObject =CamShift(bp,searchWindow,TermCriteria(TermCriteria::COUNT | TermCriteria::EPS, 10, 1));
		i_count+=1;
		
	    }
	    r=CamShift(img,searchWindow,trac_criteria);	
		*/
